...
fn default_backup_frequency_hours() -> u32 { 24 }
...
impl WorkspaceBackupConfig {
    ...
    pub fn validate(&self) -> Result<()> {
        ...
        if self.frequency_hours > 168 { // 1 week
            return Err(PixlieError::validation(
...
mod tests {
    ...
    fn test_add_pinned_objective() {
        ...
        let id = config.add_pinned_objective(
        ...
        assert_eq!(config.workspace.pinned_objectives[0].title, "Test Objective");
    }
    ...
    fn test_workspace_serialization() {
        ...
        assert_eq!(config.workspace.backup.enabled, parsed.workspace.backup.enabled);
    }
}